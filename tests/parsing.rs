...
fn version_element() {
    let f = File::open("./tests/test_data/relations.osm").unwrap();
    let osm = OSM::parse(f).unwrap();
    assert_eq!(osm.relations[&155054].version, 7);
    assert_eq!(osm.ways[&123365172].version, 3);
    assert_eq!(osm.nodes[&345579225].version, 6);
}
...
fn timestamp_element() {
    let f = File::open("./tests/test_data/relations.osm").unwrap();
    let osm = OSM::parse(f).unwrap();
    assert_eq!(osm.relations[&155054].timestamp, "2014-05-28T19:48:59Z".to_string());
    assert_eq!(osm.ways[&123365172].timestamp, "2014-10-20T20:03:35Z".to_string());
    assert_eq!(osm.nodes[&345579225].timestamp, "2012-12-10T12:46:34Z".to_string());
}
